...
const float Turret::speedAngular = MathAddon::angleDegToRad(360.0f), Turret::weaponRange = 4.0f;
...
void Turret::draw(SDL_Renderer* renderer, int tileSize) {
	drawTextureWithOffset(renderer, TextureLoader::getTextureFromPool(textureShadow), 5, tileSize);
	drawTextureWithOffset(renderer, TextureLoader::getTextureFromPool(textureMain), 0, tileSize);
}
