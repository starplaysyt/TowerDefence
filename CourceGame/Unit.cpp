...
void Unit::update(float dT, Level& level, std::vector<std::shared_ptr<Unit>>& listUnits) {
	...
	if (distanceToTarget < 0.5f) {
		healthCurrent = 0;
		AlphaRegister::isPlayerAlive = false;
		AlphaRegister::LogRegisterChanges("isPlayerAlive", "Unit.h", "Player is gone");
	}
	else {
...
void Unit::draw(SDL_Renderer* renderer, int tileSize) {
	if (renderer != nullptr) {
		...
		if (timerJustHurt.timeSIsZero() == false)
			SDL_SetTextureColorMod(TextureLoader::getTextureFromPool(texture), 255, 0, 0);
		else
			SDL_SetTextureColorMod(TextureLoader::getTextureFromPool(texture), 255, 255, 255);

		//Draw the image at the unit's position.
		int w, h;
		SDL_QueryTexture(TextureLoader::getTextureFromPool(texture), NULL, NULL, &w, &h);
		SDL_Rect rect = {
			(int)(pos.x * tileSize) - w / 2,
			(int)(pos.y * tileSize) - h / 2,
			w,
			h };
		SDL_RenderCopy(renderer, TextureLoader::getTextureFromPool(texture), NULL, &rect);
	}
}
