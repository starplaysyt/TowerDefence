...
int main(int argc, char* args[]) {
	...
	if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) < 0) { // init SDL setup
		std::cout << "Error: Couldn't initialize SDL Video or Audio = " << SDL_GetError() << std::endl;
		return 1;
	}
	else {
		//Setup the audio mixer.
		bool isSDLMixerLoaded = (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 1024) == 0); // audio mixer
		...
		if (window == nullptr) { //window setup init
			...
			if (renderer == nullptr) {
				...
				//Start the game.
				std::cout << "Init compeled" << std::endl;
